...
- variables 是 MVU 变量蓝图：为关键角色或系统状态提取变量（如好感度、信任值等），每个变量必须有 path/type/description。
- 必须提取"剧情主线进度轴"：当 stage_order 有 ≥2 个阶段时，必须输出一个 path 为 "剧情.进度"、type 为 "enum" 的变量，options 为 stage_order 全量，default 为第一个阶段。该变量将驱动剧情主线分阶段世界书的逐步展开，确保故事按阶段连贯推进。
- 必须抽取"彩蛋/隐藏内容"：当小说文本中出现"特定条件下才触发的隐藏内容"（如特定地点相遇、特定物品触发回忆、特定对话触发支线、达成隐藏条件解锁的过去），必须为每条此类内容单独生成 entry，并在 required_flags 中标注触发条件对应的 flag id（如 flag_reunion_xxx）。同时把该 flag id 输出到 reveal_flags 数组，并给出 label/description/default=false。entry 的 content 写明触发后注入的世界书正文（含揭露的设定/回忆/支线/对话引导，RP-first）。
...
输出 JSON，必须符合以下结构：
{
  ...
  "variables": [
    ...
    { "path": "剧情.进度", "type": "enum", "options": ${stageList}, "default": ${JSON.stringify(config.stageOrder[0] || '公开')}, "description": "剧情主线阶段轴，驱动分阶段世界书逐步展开" }
  ],
  "entries": [
    {
      "name": "条目名（2-6字）",
    ...
    {
      "name": "隐藏回忆",
      "content": "触发后注入的世界书正文（彩蛋/隐藏内容），写明 AI 应如何表现",
      "keys": ["触发关键词"],
      "category": "event",
      "stage": ${JSON.stringify(config.stageOrder[0] || '公开')},
      "strategy": "selective",
      "priority": 400,
      "required_flags": ["flag_hidden_memory"]
    }
  ]
}
...
- variables 必须包含 "剧情.进度" enum 变量（当 stage_order 有 ≥2 阶段时）
- required_flags: 数组，元素为 reveal_flags 中的 flag id。当且仅当这些 flag 全部为 true 时该条目才会被注入。普通条目留空数组 []；彩蛋/隐藏内容必须填入对应 flag id。
...
- 彩蛋/隐藏内容条目：写明触发后揭露的设定/回忆/支线，以及 AI 在被触发后应如何调整表现。不要把彩蛋内容直接写进常规条目。
