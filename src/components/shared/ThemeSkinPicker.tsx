  ...
  return (
    <div className="relative">
      <button
        ...
        className={compact
          ? 'w-full h-10 flex items-center justify-center rounded-lg text-slate-400 hover:text-slate-100 hover:bg-white/5 transition-colors'
          : 'w-full flex items-center justify-between px-2 py-1.5 text-xs text-slate-500 hover:text-slate-300 transition-colors'}
      ...
      {effectiveExpanded && (
        <div className={`absolute p-3 bg-slate-800/95 backdrop-blur-sm border border-slate-700 rounded-lg shadow-lg w-64 max-h-[70vh] overflow-y-auto ${compact ? 'bottom-0 left-full ml-2' : 'bottom-full left-0 right-0 mb-1'}`}>
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-medium text-slate-300">{t('themeSkin.title')}</span>
            <button
          ...
          <div className="grid grid-cols-2 gap-2">
            {THEME_PRESETS.map((preset) => (
              <button
                ...
                className="group relative rounded-lg overflow-hidden border border-slate-600 hover:border-slate-300 transition-all hover:scale-[1.02] active:scale-[0.98]"
                title={preset.name}
              >
                <img
                ...
                <div
                  className="absolute bottom-1.5 left-1.5 right-1.5 text-[10px] font-medium text-white truncate text-center"
