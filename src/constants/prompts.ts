...
格式规则（必须严格遵守）：
- "name" 字段的值必须是 "${characterName}"，绝对不能是 "{{user}}" 或其他内容
- description 中"姓名"字段必须写角色的真实姓名"${characterName}"，绝对不能写 "{{user}}"
- {{user}} 是 SillyTavern 的用户占位符，只能出现在"与{{user}}关系"这一行，禁止出现在姓名、身份或任何其他字段中
- description 必须使用 ## 标题和列表/键值对格式（不要写成散文段落）
- description 必须用第三人称写法（用角色名或"他/她"，如"许七安说话时……"或"他说话时……"，绝对不要用"你"）
- 绝对不要违背用户的原始约束
- 绝对不要写泛泛的描述（"美丽的眼睛"、"优雅的身姿"）
- 绝对不要只贴抽象性格标签而不给出具体行为衍生

✅ 正确示例："name": "${characterName}", "description": "## 基本信息\\n姓名：${characterName}\\n年龄：38岁\\n身份：城中首富谢家主母\\n与{{user}}关系：青梅竹马"
❌ 错误示例："name": "{{user}}", "description": "## 基本信息\\n姓名：{{user}}" ← 绝对禁止！
...
✅ 正确示例："name": "${characterName}", "description": "## 基本信息\\n姓名：${characterName}\\n年龄：25岁\\n身份：城南铁匠铺学徒\\n与{{user}}关系：邻居"
❌ 错误示例："name": "{{user}}" ← 绝对禁止！{{user}} 不是角色名！
